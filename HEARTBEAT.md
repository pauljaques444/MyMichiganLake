...
| **Marketplace** | ✅ Working | Browse + search + 8 category filters, create listing (5 photos, price types, condition, lake), listing detail with photo gallery + seller card + owner actions (mark sold/rented/relist/delete). **My Listings** page at `/marketplace/my-listings` shows all owner listings (active/sold/rented) with status badges and Edit/View/status-change actions. **Edit Listing** at `/marketplace/[id]/edit` pre-populates all fields + existing photos; ownership-checked at server and DB level. Map popup links still go to `/marketplace` unfiltered (low priority). |
